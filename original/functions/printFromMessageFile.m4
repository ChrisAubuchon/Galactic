; ---------------------------------------------------------------------------
; de: Offset in galactic.msg to print
; ---------------------------------------------------------------------------
printFromMSGFile:
		...
		call	fileRead
		cp	1
		jp	z, l_inline_closeAndExit
		pop	de
		ld	hl, readDMAAddress
		ld	a, e
		call	addByteToHL
		ld	b, h
		ld	c, l				; bc = File address

loc_2F72:
		ld	hl, decompMaybeBuffer
		ld	d, 7

l_printMsg_readSevenBytes:
		ld	a, (bc)
		inc	bc
		ld	(hl), a
		inc	hl
...
loc_2F92:
		...
		jp	z, loc_305D
		cp	7
		jp	z, l_inline_earthEarlyTimersB
